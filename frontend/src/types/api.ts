...
export interface ConfigView { llm_provider:string; openai_base_url: string; openai_model: string; use_local_llm: boolean; local_llm_url: string; local_llm_model: string; max_budget_usd: number; has_openai_key: boolean; has_tavily_key: boolean; status: 'ready' | 'missing' | 'degraded'; config_revision: number; role_models: Record<string, string>; credential_providers: string[] }
export type ProviderKind = 'api' | 'plan' | 'gateway'
export interface ModelProvider { id:string;name:string;region:'cn'|'global'|'local'|'custom'|'plan'|'gateway';base_url:string;api_style:string;key_url?:string|null;docs_url?:string|null;verified_at:string;kind?:ProviderKind }
export interface ConfigUpdate { llm_provider?:string; openai_api_key?: string; openai_base_url?: string; openai_model?: string; use_local_llm?: boolean; local_llm_url?: string; local_llm_model?: string; tavily_api_key?: string; max_budget_usd?: number; role_models?: Record<string, string>; provider_credentials?: Record<string, { api_key?: string; base_url?: string }> }
...
export interface TopicChatResponse { response:string; references_used:number }
export interface TopicEvaluation { specific:number;measurable:number;achievable:number;relevant:number;time_bound:number;total:number;verdict:string;suggestions:string[];references_used:number }
export interface RagIngestResult { document_id:string; filename:string; chunks:number }
...
export interface CleaningAdviceItem { index:number;action:CleaningAction;reason:string }
export interface CleaningAdviceResponse { overall:string;items:CleaningAdviceItem[];model:string }
...
export interface ChartInsightResponse { recommendation:ChartRecommendation;insight:string;caveats:string[];model:string;source:'ai'|'rules' }
