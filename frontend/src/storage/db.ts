...
interface QCDb extends DBSchema {
  ...
  project_state:{key:string;value:{projectId:string;discussionSummary:DiscussionSummary|null;chartOption:EChartsOption|null;pptResult:PPTResult|null;updatedAt:number}}
}
...
export async function clearProject(projectId:string){const db=await database;const tx=db.transaction(['datasets','chart_images','project_state'],'readwrite');for(const key of await tx.objectStore('datasets').index('by-project').getAllKeys(projectId)) await tx.objectStore('datasets').delete(key);for(const key of await tx.objectStore('chart_images').index('by-project').getAllKeys(projectId)) await tx.objectStore('chart_images').delete(key);await tx.objectStore('project_state').delete(projectId);await tx.done}
